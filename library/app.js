/* ── A:\GAMES — Library app.js (embedded in C:\PLAY) ────────────────────── */

// ── Configuration ────────────────────────────────────────────
const params = new URLSearchParams(window.location.search);
const LIBRARY_URL = params.get("library") || "https://raw.githubusercontent.com/yal-pj/dos-freeware-games-library/main/library.json";
...
function launchGame(game) {
  ...
  // When embedded in CPlay as an iframe, send message to parent
  ...
  // If a CPlay URL was passed as a query param, open game there
  if (CPLAY_URL) {
...
function setupEvents() {
  ...
  dom.openPlayerBtn?.addEventListener("click", () => {
    // When embedded in CPlay, send a message to close the library modal
    if (window.parent !== window) {
    ...
    if (CPLAY_URL) {
      ...
      window.open("https://playdosgames.xyz/", "_blank", "noopener");
    }
  });
